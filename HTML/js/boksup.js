...
users.forEach((use1r, index,) => {
  console.log(`${index + 1}번: ${use1r.name} (${use1r.age}살) (${use1r.active}엑티브 여부)(${use1r.scores}점수)(${use1r.role}롤 )`);
  use1r.age++; // 배열 요소의 원본 값 변경 가능
});

let arr1 = users.map(u => u.name);
console.log(arr1);

let arr2 = users.map(u => ({ ...u, age: u.age + 1 }));
console.log(arr2);// 기존 배열을 토대로 새로운 배열을 만드는 것이다. 10% 할인이 그 예이다.
